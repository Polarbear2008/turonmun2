...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Process Recent Data
      if (recentAppsRes.error) console.error('Error fetching applications:', recentAppsRes.error);
      setRecentApplications(recentAppsRes.data || []);

      if (recentMsgsRes.error) console.error('Error fetching messages:', recentMsgsRes.error);
      setRecentMessages(recentMsgsRes.data || []);

      // Process Status Stats
      if (statusStatsRes.error) {
        console.error('Error fetching application status stats:', statusStatsRes.error);
      } else if (statusStatsRes.data) {
        ...
        setStats(prev => ({
          ...prev,
          activeApplications: statusStatsRes.data.length,
      ...
      if (countryStatsRes.error) {
        console.error('Error fetching application country stats:', countryStatsRes.error);
      } else if (countryStatsRes.data) {
        const countryCounts: Record<string, number> = {};
        countryStatsRes.data.forEach(app => {
          if (app.country) {
      ...
      // Set mock total visitors for now
                      ...
                      {recentApplications.map((app) => (
                        <tr key={app.id} className="border-b last:border-0 hover:bg-gray-50">
                          ...
                          <td className="py-3 px-2">
                            <span className={`inline-block px-2 py-1 text-xs rounded-full ${app.status === 'approved'
