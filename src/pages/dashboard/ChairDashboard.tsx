...
const chairSidebarItems: ChairSidebarItem[] = [
  ...
  { id: 'announcements', label: 'Announcements', icon: Send, path: '/chair-dashboard/announcements' },
  { id: 'papers', label: 'Position Papers', icon: FileText, path: '/chair-dashboard/papers' },
  { id: 'messaging', label: 'Messaging', icon: MessageSquare, path: '/chair-dashboard/messaging' },
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Get chair's committee assignments
        const { data: chairAssignments, error: assignmentsError } = await supabase
        ...
        if (assignmentsError) {
          console.error('Error fetching chair assignments:', assignmentsError);
          return;
        }

        const isSuperAdmin = adminUser.role === 'superadmin';
        const committeeIds = isSuperAdmin 
          ? null // Load all committees for superadmin
          : chairAssignments?.map(ca => ca.committee_id) || [];

        // Load data - superadmin sees all, chairs see only their committees
        const queries = [];
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/60 text-sm">
                ...
                {activeTab === 'announcements' && 'Send announcements to all delegates'}
