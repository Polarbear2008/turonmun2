    ...
    >
      ...
      <motion.div variants={itemVariants} className="glass-card p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <motion.button
            className="flex items-center space-x-2 bg-gold-500 hover:bg-gold-600 px-4 py-2 text-white transition-colors rounded-lg"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {documents.map((doc, index) => (
            <div key={index} className="glass-panel p-4 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-xs">{doc.size} • {doc.type}</p>
                  </div>
                </div>
              ...
              <p className="text-xs text-white/50">Uploaded on {doc.uploadDate}</p>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="glass-card p-6">
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <p className="text-white/60 text-sm">Conference registration fee</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="glass-panel p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold text-green-400">${paymentInfo.amount} {paymentInfo.currency}</p>
          </div>
          
          <div className="glass-panel p-4 rounded-lg">
            <p className="text-white/70 text-sm mb-1">Payment Status</p>
            <div className="flex items-center space-x-2">
              <CheckCircle className="h-4 w-4 text-green-400" />
              <p className="text-green-400 font-medium capitalize">{paymentInfo.status}</p>
            </div>
          </div>

          <div className="glass-panel p-4 rounded-lg">
            <p className="text-white/70 text-sm mb-1">Transaction ID</p>
            <p className="text-white font-mono text-sm">{paymentInfo.transactionId}</p>
          </div>
        </div>

        <div className="mt-4 pt-4 border-t border-white/10">
          <div className="flex items-center justify-between text-sm">
            <span className="text-white/70">Payment Date: {paymentInfo.paymentDate}</span>
            <span className="text-white/70">Method: {paymentInfo.method}</span>
          </div>
        </div>
      </motion.div>
    </motion.div>
